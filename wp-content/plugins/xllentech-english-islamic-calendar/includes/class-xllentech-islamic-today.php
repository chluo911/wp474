...
if ( !class_exists( 'Xllentech_Islamic_Today' ) ) {
	...
	class Xllentech_Islamic_Today extends WP_Widget {
		...
		function form($instance) {
			...
			<p>
			<label for="<?php _e( $this->get_field_id('title') ); ?>"><?php _e( 'Widget Title', 'wp_widget_plugin' ); ?></label>
		...
		private function send_reminder_email( $today_islamic_date ) {
			...
			if( $this->xc_options['calendar_email_choice'] == "Yes" ) {
				...
				if( $this->xc_options['days_email_sent'] == 0 && $today_islamic_date[0] >= 29 ) {
					...
					<form name='form29days' action='".$site_url."/days?year_number=" . $today_islamic_date[2] . "&month_number=" . $today_islamic_date[1] . "&islamic_days=29' type='post'>
					...
					<td align><form action='".$site_url."/days?year_number=" . $today_islamic_date[2] . "&month_number=" . $today_islamic_date[1] . "&islamic_days=30' name='form30days' type='post'>
		...
		//	$english_currentmonth_days=date_format($english_currentdate,'t');
			...
			if( $islamic_date_array == NULL ) {
				...
				<div class='xllentech-islamic-today'>
					<span class='xllentech-today-english-date-LTR xllentech_english_<?php _e( $this->xc_options['xc_color_theme'] ); ?>'>
				
					<?php _e( $english_currentday ."-". $english_currentmonth_name ."-". $english_currentyear ); ?>
					</span>
				
					<span class='xllentech-today-spacing'></span>
					<span class='xllentech-today-islamic-date-LTR xllentech_islamic_<?php _e( $this->xc_options['xc_color_theme'] ); ?>'>
						<?php _e( $islamic_date_array[0] ."-". $islamic_months[ $islamic_date_array[1] ] ."-". $islamic_date_array[2] ); ?>
