...
class Xllentech_English_Islamic_Calendar extends WP_Widget {
	...
	<p>
		...
		<input class="widefat" id="<?php _e( $this->get_field_id('title') ); ?>" name="<?php _e( $this->get_field_name('title') ); ?>" type="text" value="<?php _e( $title, 'xllentech-calendar' ); ?>" />
	</p>
	<?php
	}
	   ...
	   if ( $title ) {
		...
		if( count($islamic_date_data) <= 0 ) {
			...
			if( count($islamic_date_data) <= 0 ) {
				...
				echo $err_msg->get_error_message();
				echo $after_widget;
			//	_e( $output_calendar );
				return;
			}
		...
		try {
			if ( is_plugin_active( 'xllentech-calendar-pro/xllentech-calendar-pro.php' ) ) {
				...
				$filepath = ABSPATH . 'wp-content/plugins/xllentech-calendar-pro/public/partials/pro-calculate-dates.php';
				...
				$filepath = ABSPATH . 'wp-content/plugins/xllentech-calendar-pro/public/templates/display-calendar-pro-body.php';
				if( file_exists( $filepath ) )
					require $filepath;
				else {
					_e( 'Required file missing in Xllentech Calendar Pro ADD-ON, You may have older version, Please update your ADD-ON or contact support.');
