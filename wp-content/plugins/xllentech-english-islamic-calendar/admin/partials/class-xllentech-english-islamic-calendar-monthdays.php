...
Class Xllentech_English_Islamic_Calendar_Monthdays {
	...
	function xeic_month_days_validate_values( $options ) {
		...
		$validated_options['year_number'] = filter_var( $options["year_number"], FILTER_VALIDATE_INT );
		if( ! $validated_options['year_number'] ) return NULL;
		
		$validated_options['month_number'] = filter_var( $options["month_number"], FILTER_VALIDATE_INT );
		if( ! $validated_options['month_number'] ) return NULL;
		
		$validated_options['islamic_days'] = filter_var( $options["islamic_days"], FILTER_VALIDATE_INT );
	...
	<?php
		if( isset($_POST["add_monthdays"]) ) {
			...
			if( ! empty( $_POST ) && check_admin_referer( 'xc-settings-delete-month-days-action', 'xc-settings-delete-month-days-nonce' ) && current_user_can( 'manage_options' ) ) {
					...
					$wpdb->delete( $month_days_table, array( 'month_number' => $_POST["month_number"],
						...
						<div class="inside">
							...
							<form method="post" action="#">
								<ul class="xllentech-month-days">
									...
									<li><label for="year_number">Year:</label><input value="1438" type="number" name="year_number"/></li>
					...
					<?php 
						$month_data = $wpdb->get_results( "SELECT * FROM $month_days_table ORDER BY year_number DESC, month_number DESC" );
							foreach( $month_data as $islamic_date_data ) {
								?> 
									<tr>
										<td> <?php _e( $islamic_date_data->month_number ); ?></td>
										<td> <?php _e( $islamic_date_data->year_number ); ?></td>
										<td> <?php _e( $islamic_date_data->days ); ?></td>
								
										<td>
											<form method="post" action="#">
												<input type="hidden" name="month_number" value="<?php _e( $islamic_date_data->month_number ); ?>">
												<input type="hidden" name="year_number" value="<?php _e( $islamic_date_data->year_number ); ?>">
