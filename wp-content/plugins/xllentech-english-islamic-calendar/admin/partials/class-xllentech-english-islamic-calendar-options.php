...
Class Xllentech_English_Islamic_Calendar_Options {
	...
	function xeic_options_validate_values( $options ) {
		...
		$validated_options['xc_first_day'] = filter_var( $_POST['xc_first_day'], FILTER_SANITIZE_STRIPPED );
		$validated_options['xc_color_theme'] = filter_var( $_POST['xc_color_theme'], FILTER_SANITIZE_STRIPPED );
		$validated_options['xc_page_pin'] = filter_var( $_POST['page_pin'], FILTER_VALIDATE_INT );
	...
	function xeic_save_options( $options ) {
		...
		return update_option( 'xc_options', array_merge( $xc_options, array_intersect_key( $options, $xc_options ) ) );
	}
						...
						<div id="section1" class="postbox">
							...
							<ul class="xllentech-options">
								<li>
									...
									?>
										<option value="<?php echo $timezone_list; ?>" <?php if( $timezone_list == $xc_options['xc_time_zone'] ) _e( 'selected' ); ?> ><?php _e( $timezone_list ); ?></option>
										<?php } ?>
									</select>
								</li>
								<li>
									...
									<?php if($xc_options['xc_first_day']=='Sunday') _e( 'checked="checked"' ); ?>>
									<?php esc_attr_e('Sunday'); ?>&nbsp;&nbsp;&nbsp;</input>

									<input type="radio" name="xc_first_day" value="Monday" class="theme_select" 
									<?php if($xc_options['xc_first_day']=='Monday') _e( 'checked="checked"' ); ?>>
									<?php esc_attr_e('Monday'); ?></input>
									
								</li>
								<li>
									<label><strong><?php _e( 'Color Theme', 'xllentech-english-islamic-calendar' ); ?>: </strong></label>
									<input type="radio" name="xc_color_theme" value="Default" class="theme_select" 
									<?php 
										if( $xc_options['xc_color_theme'] == 'Default' ) 
											_e( 'checked="checked"' ); 
									?>>Default<span class="theme_color_box" style="background-color:#838487;"></span>
									
									<input type="radio" name="xc_color_theme" value="Blue" class="theme_select" 
									<?php 
										if( $xc_options['xc_color_theme'] == 'Blue' ) 
											_e( 'checked="checked"' ); 
									?>>Blue<span class="theme_color_box" style="background-color:#4c6a92;"></span>
									
									<input type="radio" name="xc_color_theme" value="Red" class="theme_select" 
									<?php 
										if( $xc_options['xc_color_theme'] == 'Red' ) 
											_e( 'checked="checked"' ); 
									?>>Red<span class="theme_color_box" style="background-color:#b93a32;"></span>
									
									<span style="white-space:nowrap">
										<input type="radio" name="xc_color_theme" value="Green" class="theme_select" 
										<?php 
											if( $xc_options['xc_color_theme'] == 'Green' ) 
												_e( 'checked="checked"' ); 
								...
								<li>
									...
									<input type='checkbox' name='xeic_email_choice' id='xeic_email_choice' 
								<?php if( $xc_options['calendar_email_choice'] != 'No' || $xc_options['calendar_email_choice'] == 1 ): _e( 'value=1 checked' ); else: _e( 'value=0' ); endif; ?> />
								</li>
								<li>
									<label><strong><?php _e( 'Calendar Admin Email', 'xllentech-english-islamic-calendar' ); ?>: </strong></label>
									<input type='text' name='admin_email' id='admin_email' value='<?php _e( $xc_options['calendar_admin_email'] ); ?>' style="width: 220px;"/>
								</li>
								<li>
									<label><strong><?php _e( 'Page PIN', 'xllentech-english-islamic-calendar' ); ?>: </strong></label>
									<input type='number' required size="6" maxlength="6" name="page_pin" id="page_pin" value="<?php _e( $xc_options['xc_page_pin'] ); ?>" />
								</li>
								<li>
						...
						<div id="section2" class="postbox">
							...
							<div class="inside">
								<div class="tab3-sample-email">
									...
									<form name='form29days' action="<?php _e( esc_url($site_url) ); ?>/days" method='get' target="_blank">
									<input name='islamic_days' type='hidden' value='29'>
									<input name='month_number' type='hidden' value='2'>
									<input name='year_number' type='hidden' value='1438'>
									<button class='pure-button' type='submit' style='width:60%;height:35px;background:rgb(28,184,65);font-size: 125%;'>29 Days</button>
									</form></td><td></td>
									<td align><form name='form30days' action="<?php _e( esc_url($site_url) ); ?>/days" method='get' target="_blank">
