		...
		if( isset( $_POST["update_xc_troubleshooting"] ) && current_user_can( 'manage_options' ) ) {
			...
			if( !empty( $_POST ) && check_admin_referer( 'xc-troubleshooting-add-date-action','xc-troubleshooting-add-date-nonce' ) ) {
				...
				if( $validated_options != NULL ) {
					...
					'english_year' => $validated_options["english_year"], 'islamic_day' => $validated_options["islamic_day"], 'islamic_month' => $validated_options["islamic_month"], 'islamic_year' => $validated_options["islamic_year"] ), array('%d','%d','%d','%d','%d') );
							...
							<form method="post" action="#">
								<ul class="xllentech-troubleshooting">
									<li>
										...
										if($xc_options['xc_data_uninstall']!='No'): _e( 'value="Yes" checked' ); else: _e( 'value="No"' ); endif; 
										?> />
									</li>
									<li>
				...
				<?php 
					$month_data = $wpdb->get_results( "SELECT * FROM $month_firstdate_table ORDER BY english_year DESC, english_month DESC" );
						foreach( $month_data as $islamic_date_data ) {
							?>
								<tr>
									<td><?php _e( $islamic_date_data->english_month ); ?></td>
									<td><?php _e( $islamic_date_data->english_year ); ?></td>
									<td><?php _e(  $islamic_date_data->islamic_day ); ?></td>
									<td><?php _e(  $islamic_date_data->islamic_month ); ?></td>
									<td><?php _e( $islamic_date_data->islamic_year ); ?></td>

									<td>
										<form method="post" action="#">
											<input type="hidden" name="month_number" value="<?php echo $islamic_date_data->english_month; ?>">
											<input type="hidden" name="year_number" value="<?php echo $islamic_date_data->english_year; ?>">
