...
class Xllentech_English_Islamic_Calendar_Public {
	...
	public function calculate_islamic_date( $english_month, $stack ) {
		...
		if( $stack > 12 ) {
			...
			echo "<strong>Xllentech Calendar Error: Islamic Date could not be found in the database. Please contact support or Add islamic date in the database from the Xllentech Calendar Settings -> Troubleshooting page.</strong>";
			return;
		}
		global $wpdb;
	...
	public function xc_display_base_header( $english_currentmonth, $english_currentyear, $islamic_firstmonth, $islamic_firstyear ) { 
		...
		?>
		<table border='1' class='xc_table_<?php _e( $xc_options['xc_color_theme'] ); ?>'>
				...
				<tr class='xllentech-main-nav xllentech_header'>
					<th colspan='7'>
						<div class='xllentech-month-names'>
							<span class='xllentech-english-month'><?php _e( $english_currentmonth .' '. $english_currentyear ); ?></span>
							<span class='xllentech-islamic-month'><?php _e( $islamic_firstmonth .' '. $islamic_firstyear ); ?></span>
						</div>
					</th>
				</tr>
		...
		<?php 	for ($c=0; $c<35; $c++) { ?>
					<td>
						<div class='xllentech-daybox'>
							<span class='<?php _e( $xllentech_english_css[$c] ); ?>'><?php _e( $english_day_sequence[$c] ); ?></span>
							<span class='xllentech-spacing'></span>
							<span class='<?php _e( $xllentech_islamic_css[$c] ); ?>'><?php _e( $islamic_day_sequence[$c] ); ?></span>
						</div>
					</td>
