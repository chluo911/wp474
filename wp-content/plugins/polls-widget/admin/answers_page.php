            ...
            <table class="wp-list-table widefat fixed pages" style="width:95%">
                ...
                <tbody>
                 <tr ng-repeat="rows in names | filter:filtering_table" class="description_row">
						 ...
						 <td><a href="admin.php?page=Polls&task=duplicate_poll&id={{rows.id}}">Duplicate</a></td>
...
<?php
	...
	private function duplicate($id){
		global $wpdb;
		$orginal_poll_question=$wpdb->get_row('SELECT * FROM '.$wpdb->prefix.'polls_question WHERE id='.$id);
		$orginal_answers=$wpdb->get_results('SELECT * FROM '.$wpdb->prefix.'polls WHERE question_id='.$id." ORDER BY `answer_name` ASC");
		$save_or_no=$wpdb->insert( $wpdb->prefix.'polls_question', 
			array( 
				'question' => $orginal_poll_question->question,
				'name' =>$orginal_poll_question->name.'_copy',
				'answer_type' => $orginal_poll_question->answer_type,
		...
		if($save_or_no){
			$id_question=$wpdb->get_var('SELECT MAX(id) FROM '.$wpdb->prefix.'polls_question');
			foreach($orginal_answers as $answer){
				if($answer){
					$save_or_no=$wpdb->insert( $wpdb->prefix.'polls', 
						array( 
							'question_id' => $id_question,
		...
		if($save_or_no){
			...
			?><div id="message" class="error"><p>Item Not Duplicated</p></div> <?php
		}
	}
