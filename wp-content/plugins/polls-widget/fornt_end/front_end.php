...
class poll_front_end {
	...
	function __construct($params) {
		self::$prefix_main_poll = 0;
		$this->menu_name = $params['menu_name'];
		if (isset($params['databese_parametrs']))
		...
		if ($user_information->user_ip) {
			if ($array_of_answered_user)
				foreach ($array_of_answered_user as $answer) {
					$wpdb->query('UPDATE ' . $wpdb->prefix . 'polls SET vote = vote-1 WHERE `question_id` = ' . $question_id . ' AND `answer_name` = ' . intval($answer) . '');
				}
			if ($new_voted_array)
				foreach ($new_voted_array as $new_answer) {
					$wpdb->query('UPDATE ' . $wpdb->prefix . 'polls SET vote = vote+1 WHERE `question_id` = ' . $question_id . ' AND `answer_name` = ' . intval($new_answer) . '');
				}
			$wpdb->update(
				$wpdb->prefix . 'polls_users',
				array(
	...
	private function generete_html_front_end_poll($poll_answer_id) {
		...
		if (isset($user_information->answers) && $user_information->answers != null) {
		...
		foreach ($answers as $answer) {
			...
			$poll_question_and_answers .= '<span class="radio_or_select_span"><input onclick="send_to_databese(' . $poll_answer_id . ',' . self::$prefix_main_poll . ')" class="poll_inputs" type="' . $answer_type . '" name="poll_' . self::$prefix_main_poll . '" value="' . $answer->answer_name . '" ' . checked(1, $checked, false) . '></span>';
			$poll_question_and_answers .= '<div class="div_only_answer">';
			$poll_question_and_answers .= '<div class="main_div_of_the_poll_answer" onclick="clicked_in_poll_div(this,' . $poll_answer_id . ',' . self::$prefix_main_poll . ')" title="' . $answer->answer . '">';
	...
	public function generete_front_end_single_poll($poll_answer_id, $poll_design_id) {
		...
		$poll_front_end_code .= $this->genrerte_styles_for_curent_poll(self::$prefix_main_poll, $poll_design_row);
		$poll_front_end_code .= '<div class="poll_main_div_' . self::$prefix_main_poll . ' poll_min_div_cur_answer_' . $poll_answer_id . ' wpdevartpoll">' . $this->generete_html_front_end_poll($poll_answer_id) . '</div>';
		return $poll_front_end_code;
	}
